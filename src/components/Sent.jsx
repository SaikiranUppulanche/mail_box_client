...
import { deleteSentMail, fetchSent } from "../store/mailActions";
...
const Sent = () => {
  ...
  const readHandler = (mail) => {
    navigate(`sent-message/${mail.id}`);
  };

  const handleDelete = (mail) => {
    dispatch(deleteSentMail(mail.id));
  };

  return (
    <ul>
      {sentMailData &&
        sentMailData.toReversed().map((item) => (
          <li
            key={item.id}
          ...
          >
            ...
            >
              <div className="flex items-center">
                <div className="flex items-center mr-3 ml-4 space-x-1 text-gray-500">
                  To:
                </div>
